...
- For the coda envelope, we only analyze data from S wave arrival to ends of coda. Thus, we only focus on validating the S phase arrival. The S wave arrival is being validated with envelope peak. The arrival should earlier than the wave maximum, so we remove picks that tpeak-ts is negative and also we remove data with SNR below 4. However from
